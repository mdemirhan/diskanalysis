...
def render_summary(
    ...
    *,
    ...
    table.add_section()
    extra_cols = 1 + int(apparent_size)
...
def render_focused_summary(
    ...
    *,
    ...
    if top_temp:
        ...
        console.print(
            _insights_table(
                "Largest Temporary Files/Directories", insights, top_n, root_prefix, apparent_size=apparent_size
            )
        )
    if top_cache:
        ...
        console.print(
            _insights_table(
                "Largest Cache Files/Directories", insights, top_n, root_prefix, apparent_size=apparent_size
            )
        )
    ...
    if top_dirs:
        console.print(
            _top_nodes_table(
                "Largest Directories", root, top_n, NodeKind.DIRECTORY, root_prefix, apparent_size=apparent_size
            )
        )
    if top_files:
        console.print(
            _top_nodes_table("Largest Files", root, top_n, NodeKind.FILE, root_prefix, apparent_size=apparent_size)
        )
